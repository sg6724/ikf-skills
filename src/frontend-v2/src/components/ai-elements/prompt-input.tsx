...
const TextValueContext = createContext<{ value: string; setInput: (v: string) => void } | null>(null);
...
export const PromptInput = ({
  ...
  return (
    <LocalAttachmentsContext.Provider value={attachmentsCtx}>
      <TextValueContext.Provider value={textCtx}>
...
export const PromptInputTextarea = ({
  ...
  const textCtx = useContext(TextValueContext);
  ...
  const controlledProps = controller
    ...
    : {
      value: textCtx?.value,
      onChange: (e: ChangeEvent<HTMLTextAreaElement>) => {
        textCtx?.setInput(e.currentTarget.value);
        onChange?.(e);
      },
    };
...
export const PromptInputFooter = ({
  ...
  <InputGroupAddon
    align="block-end"
    className={cn("flex flex-row items-center justify-between gap-1 px-3 pb-3 pt-0", className)}
    {...props}
  />
);
...
export const PromptInputSubmit = ({
  ...
  const controller = useOptionalPromptInputController();
  const textCtx = useContext(TextValueContext);
  const value = controller?.textInput.value ?? textCtx?.value ?? "";
  const isDisabled = props.disabled || (!value.trim() && !isGenerating);
